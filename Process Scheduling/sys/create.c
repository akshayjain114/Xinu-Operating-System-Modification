					...
					/* array in the code)		*/
{
	...
	/*Additional steps for linux scheduling*/
	if(getschedclass() == LINUXSCHED){
		linuxprocesses[pid].quantum = 0;
		linuxprocesses[pid].priority = -1; //Processes created in middle of epoch should not be run until next epoch
	}
