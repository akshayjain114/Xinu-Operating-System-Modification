...
LOCAL		sysinit();
LOCAL		initialize_global_page_tables();
...
int currentVirtualPage = VIRTUALPAGE0;
fr_map_t frm_tab[NFRAMES];
bs_map_t bsm_tab[NBACKINGSTORES];

/* Added for SC page replacement policy */
struct scqueue *schead = NULL;
struct scqueue *scprev = NULL;
int policydebugging = 0;
...
nulluser()				/* babysit CPU when no one is home */
{
	...
	initialize_directory( &((&proctab[0])->pdbr), 0 );
	write_cr3( (&proctab[0])->pdbr );
	initialize_global_page_tables( (unsigned int *)(&proctab[0])->pdbr );
	enable_paging();
